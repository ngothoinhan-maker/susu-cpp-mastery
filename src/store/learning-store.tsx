...
**Lưu ý quan trọng:** Vòng lặp lồng $O(N^2)$ chỉ dùng được với $N \le 10^4$. Với $N = 10^5$ thì $N^2 = 10^{10}$ → TLE!
...
- [ ] Kiểm tra giới hạn bộ nhớ: $N \le 10^7$ là an toàn`,
        homeworkProblems: [
          {
            id: "w3-hw1",
...
- [ ] Với $A, B \le 10^{18}$: Euclid vẫn chạy tốt (max ~87 bước)`,
        homeworkProblems: [
          {
            id: "w3-hw2",
...
| Kích thước tối đa | Biết trước ($\le 10^7$) | Tùy heap |
...
1.  Điểm thi nào bắt đầu từ mốc 3 trở lên? (Vị trí đầu tiên $\ge 3$).
...
| :--- | :--- | :--- | :--- | :--- |
| \`lower_bound\` | 20 | \`a[1]\` (giá trị 20) | 1 | Phần tử đầu tiên $\ge 20$ |
| \`upper_bound\` | 20 | \`a[4]\` (giá trị 30) | 4 | Phần tử đầu tiên $> 20$ |
| \`lower_bound\` | 25 | \`a[4]\` (giá trị 30) | 4 | Phần tử đầu tiên $\ge 25$ |
| \`upper_bound\` | 25 | \`a[4]\` (giá trị 30) | 4 | Phần tử đầu tiên $> 25$ |
| \`lower_bound\` | 50 | \`a.end()\` | 6 (kích thước mảng) | Không có số nào $\ge 50$ |
...
*   Lưới ô vuông thường có kích thước nhỏ ($N, M \le 500$).
...
Hãy tưởng tượng bạn có mảng gồm $N$ số nguyên ($N \le 10^5$) và cần trả lời $Q$ câu hỏi dạng: *"Hãy tính tổng các số từ vị trí $L$ đến $R$?"* ($Q \le 10^5$).
...
Tính $A^B \pmod{M}$ với $B \le 10^{18}$ trong độ phức tạp thời gian $O(\log B)$.
...
> *   *Cách khắc phục:* Luôn đánh giá số trạng thái tối đa trước khi code Backtracking. Chỉ dùng khi $N \le 10$.
...
*   Kích thước đầu vào $N$ rất nhỏ ($N \le 12$).
...
> Duyệt lại toàn bộ các quân hậu đã đặt để kiểm tra chéo (mất độ phức tạp $O(\text{row})$ mỗi lượt) thay vì sử dụng 3 mảng đánh dấu trạng thái $O(1)$. Điều này làm chương trình chạy rất chậm khi $N \ge 11$.
...
*   **Nguyên lý:** Kích thước cửa sổ $[L, R]$ co giãn tự động để thỏa mãn một điều kiện nào đó (ví dụ tổng các phần tử $\le S$).
...
### Bước 2 – Cửa sổ biến động: Tìm dãy con liên tục dài nhất có tổng $\le K$
...
*   Kích thước đầu vào rất lớn ($N \le 10^6$) $\rightarrow$ Quy hoạch động $O(N^2)$ chắc chắn TLE, gợi ý phải dùng Greedy $O(N \log N)$.
...
Khi $N \le 10^5$, thuật toán $O(N^2)$ ở trên sẽ chạy mất khoảng 10 giây và bị TLE. Ta cần một hướng tiếp cận khác: duy trì một mảng \`tails\` lưu trữ các phần tử kết thúc nhỏ nhất có thể của các dãy con tăng có độ dài khác nhau.
...
*   Nếu không, ta thay thế phần tử đầu tiên trong \`tails\` mà $\ge x$ bằng giá trị \`x\` (kỹ thuật tối ưu hóa biên).
...
> Đề bài có thể yêu cầu dãy con tăng nghiêm ngặt ($a[i] > a[j]$) hoặc không giảm ($a[i] \ge a[j]$). 
> *   *Cách khắc phục:* 
...
- [ ] Dùng $O(N^2)$ khi $N \le 2000$, bắt buộc dùng $O(N \log N)$ khi $N \le 10^5$.
...
*   Với mỗi vật $i$:
    ...
    *   **Trường hợp 2 (Chọn vật i - nếu sức chứa hiện tại $j \ge w[i]$):** \`dp[i][j] = dp[i-1][j - w[i]] + v[i]\` (Lấy giá trị vật $i$ cộng với giá trị tối ưu của phần túi còn dư \`j - w[i]\`).
...
- [ ] Phân biệt được sự khác nhau giữa bài toán cái túi 0/1 (mỗi vật chọn $\le 1$ lần) và cái túi vô hạn.
...
| :--- | :--- | :--- |
| $N \le 10$ | $O(N!)$ hoặc $O(3^N)$ | Đệ quy quay lui liệt kê |
| $N \le 20$ | $O(2^N)$ | Sinh tập con, Bitmask |
| $N \le 500$ | $O(N^3)$ | Quy hoạch động Floyd-Warshall |
| $N \le 2000$ | $O(N^2)$ | Quy hoạch động 2D, duyệt mảng lồng |
| $N \le 10^5$ | $O(N \log N)$ | Sắp xếp, Binary Search, Fenwick/Segment Tree |
| $N \le 10^7$ | $O(N)$ hoặc $O(\log N)$ | Duyệt mảng 1 lần, Lũy thừa nhị phân |
