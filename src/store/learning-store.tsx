...
### ⚡ Khái niệm bổ trợ: Các toán tử so sánh trong C++

Để so sánh hai giá trị với nhau, C++ sử dụng các **toán tử so sánh**. Kết quả của phép so sánh luôn trả về đúng (\`true\`) hoặc sai (\`false\`).
...
> [!CAUTION]
> **Bẫy thường gặp:** Phép so sánh bằng bắt buộc phải dùng **hai dấu bằng \`==\`**. Nếu bạn chỉ viết một dấu bằng \`=\`, chương trình sẽ hiểu đó là phép gán giá trị (ví dụ: gán giá trị 5 cho biến) chứ không phải so sánh!
...
Hãy nghĩ về bảng xếp hạng giải bóng đá Ngoại hạng Anh: Các đội tuyển được sắp xếp vị trí dựa trên một loạt các quy tắc ưu tiên xếp chồng lên nhau:
1.  **Số điểm** cao hơn sẽ đứng trước.
2.  Nếu hai đội bằng điểm nhau: đội nào có **hiệu số bàn thắng bại** tốt hơn đứng trước.
3.  Nếu vẫn bằng nhau: đội nào **ghi nhiều bàn thắng** hơn đứng trước.

Để thực hiện kiểu sắp xếp phức tạp như thế này trong C++, ta không thể chỉ dùng các toán tử so sánh mặc định như lớn hơn \`>\` hay nhỏ hơn \`<\`. Chúng ta cần gom nhóm các thuộc tính của đối tượng vào một kiểu dữ liệu tự định nghĩa (**Struct**), rồi hướng dẫn hàm sắp xếp cách so sánh chúng bằng cách viết một **Hàm so sánh riêng (Custom Comparator)**. Kỹ thuật này là chìa khóa để giải quyết mọi bài toán quản lý và sắp xếp thông tin đa tiêu chí trong thực tế.
...
Trong C++, \`struct\` cho phép bạn đóng gói nhiều biến thành viên có kiểu dữ liệu khác nhau (ví dụ: họ tên dạng \`string\`, điểm số dạng \`double\`, mã số dạng \`int\`) vào một thực thể duy nhất. Điều này giúp mã nguồn gọn gàng và mô phỏng thực tế trực quan hơn.

### 2. Hàm so sánh (Comparator)
Hàm so sánh là một hàm nhận vào hai đối tượng \`a\` và \`b\` cùng kiểu dữ liệu, có dạng chung:
\`\`\`cpp
bool cmp(const T& a, const T& b);
\`\`\`
Hàm này sẽ trả về \`true\` nếu đối tượng \`a\` đứng **trước** đối tượng \`b\` trong kết quả sắp xếp cuối cùng, ngược lại trả về \`false\`.

> [!IMPORTANT]
> **Quy tắc Strict Weak Ordering (Bắt buộc):**
> Hàm comparator của bạn **phải trả về false khi hai phần tử bằng nhau** (ví dụ: nếu \`a == b\` thì bắt buộc phải trả về \`false\`). Nếu bạn cố tình dùng các toán tử như \`>=\` hay \`<=\` trong hàm comparator (ví dụ: \`return a.score >= b.score;\`), trình biên dịch có thể gặp lỗi vòng lặp vô hạn, gây crash chương trình hoặc lỗi tràn bộ nhớ (**Segmentation Fault**)! Hãy luôn luôn chỉ sử dụng dấu lớn hơn hẳn \`>\` hoặc nhỏ hơn hẳn \`<\`.
...
**Giới thiệu mã nguồn:**
Dưới đây là chương trình C++ hoàn chỉnh định nghĩa cấu trúc \`Student\` chứa thông tin học sinh và sử dụng một hàm so sánh rời tên là \`compareStudents\` để sắp xếp danh sách học sinh theo thứ tự: Điểm số giảm dần; nếu trùng điểm thì học sinh nào có mã số \`id\` nhỏ hơn (đăng ký trước) sẽ đứng trước.
...
// Định nghĩa cấu trúc Student để đóng gói thông tin học sinh
struct Student {
    string name; // Họ và tên
    int score;   // Điểm số (0 đến 10)
    int id;      // Mã định danh duy nhất (ID)
};

// Hàm comparator tự định nghĩa: Trả về true nếu học sinh a đứng trước học sinh b
bool compareStudents(const Student& a, const Student& b) {
    // Tiêu chí 1: Ưu tiên điểm cao hơn đứng trước (giảm dần)
    if (a.score != b.score) {
        return a.score > b.score; 
    }
    // Tiêu chí 2: Khi điểm bằng nhau, ưu tiên ID nhỏ hơn đứng trước (tăng dần)
    return a.id < b.id; 
}

int main() {
    // Khởi tạo danh sách gồm 3 học sinh chưa được sắp xếp
    vector<Student> list = {
        {"An", 9, 3},
        {"Binh", 10, 1},
        {"Ca", 9, 2}
    ...
    // Gọi hàm sort và truyền hàm compareStudents làm tiêu chí so sánh
...
**Giải thích chi tiết từng câu lệnh:**
*   \`struct Student { ... };\`: Định nghĩa kiểu cấu trúc tên là \`Student\` chứa các biến thành viên \`name\`, \`score\` và \`id\`. Lưu ý luôn có dấu chấm phẩy \`;\` ở cuối định nghĩa struct.
*   \`bool compareStudents(const Student& a, const Student& b)\`: Hàm comparator nhận vào hai tham chiếu hằng \`const Student&\` đại diện cho hai học sinh cần so sánh. Tham chiếu giúp tránh sao chép dữ liệu để tối ưu tốc độ, còn \`const\` đảm bảo dữ liệu gốc không bị thay đổi trong quá trình sắp xếp.
*   \`if (a.score != b.score) return a.score > b.score;\`: Nếu điểm của học sinh \`a\` khác điểm của học sinh \`b\`, ta sắp xếp theo điểm giảm dần bằng phép so sánh lớn hơn \`>\`.
*   \`return a.id < b.id;\`: Dòng này chỉ thực thi khi điều kiện ở khối lệnh \`if\` phía trên không thỏa mãn (tức là \`a.score == b.score\`). Khi đó, ta tiến hành so sánh \`id\` của hai học sinh theo thứ tự tăng dần bằng phép so sánh nhỏ hơn \`<\`.
*   \`sort(list.begin(), list.end(), compareStudents);\`: Ta truyền tên hàm comparator \`compareStudents\` làm tham số thứ ba cho hàm \`std::sort\`. Chương trình sẽ tự động áp dụng logic của hàm này để hoán đổi vị trí các học sinh.
*   \`const auto& s : list\`: Cú pháp vòng lặp range-based kết hợp tham chiếu hằng giúp duyệt qua từng phần tử một cách nhanh chóng và an toàn.

**Trace với danh sách ban đầu:**
*   So sánh "An" (9 điểm, ID 3) và "Binh" (10 điểm, ID 1) → \`10 != 9\` → Binh có điểm cao hơn → Binh đứng trước.
*   So sánh "An" (9 điểm, ID 3) và "Ca" (9 điểm, ID 2) → điểm bằng nhau (\`9 == 9\`) → so sánh ID: ID của Ca (\`2\`) nhỏ hơn ID của An (\`3\`) → Ca đứng trước.
*   Kết quả sắp xếp cuối cùng: Binh (10 điểm, ID 1) → Ca (9 điểm, ID 2) → An (9 điểm, ID 3). ✓
...
**Giới thiệu mã nguồn:**
Thuật toán \`std::sort\` mặc định sử dụng thuật toán lai Introsort, thuật toán này không bảo toàn thứ tự xuất hiện ban đầu của các phần tử bằng nhau (không ổn định). Nếu bài toán có yêu cầu nghiêm ngặt: *"Hãy sắp xếp mảng theo tiêu chí mới, nhưng nếu các phần tử bằng nhau theo tiêu chí đó thì phải giữ nguyên thứ tự xuất hiện ban đầu của chúng trong mảng"* thì bạn bắt buộc phải dùng **std::stable_sort**. Dưới đây là chương trình minh họa sắp xếp dãy số nguyên theo tổng chữ số tăng dần và giữ nguyên thứ tự xuất hiện gốc của các số có cùng tổng chữ số.
...
int getDigitSum(int n) {
    int sum = 0;
    while (n > 0) {
        sum += n % 10; // Lấy chữ số hàng đơn vị cộng vào tổng
        n /= 10;      // Bỏ chữ số hàng đơn vị
    }
    return sum;
}
...
int main() {
    int n = 5;
    // Các số 23, 32, 41, 14 đều có tổng chữ số bằng 5
    vector<int> a = {15, 23, 32, 41, 14};
    
    // Sử dụng stable_sort thay vì sort thông thường để bảo toàn thứ tự ban đầu
...
**Giải thích chi tiết từng câu lệnh:**
*   \`int getDigitSum(int n)\`: Hàm phụ thực hiện thuật toán tính tổng chữ số của số nguyên dương \`n\` bằng cách chia lấy dư cho 10 để lấy chữ số cuối và chia nguyên cho 10 để bỏ chữ số cuối, lặp liên tục cho đến khi \`n\` bằng 0.
*   \`stable_sort(a.begin(), a.end(), [](int x, int y) { ... })\`: Hàm sắp xếp ổn định. Khi so sánh phần tử \`x\` và \`y\`, nó sẽ gọi hàm \`getDigitSum\` để so sánh tổng các chữ số.
*   **Phân tích cơ chế chạy:**
    *   Tổng chữ số của các số lần lượt là: \`15\` (tổng 6), \`23\` (tổng 5), \`32\` (tổng 5), \`41\` (tổng 5), \`14\` (tổng 5).
    *   Do ta muốn sắp xếp tăng dần theo tổng chữ số, các số có tổng chữ số là \`5\` sẽ đứng trước số có tổng chữ số là \`6\`.
    *   Vì các số \`23\`, \`32\`, \`41\`, \`14\` đều có tổng là 5 nên độ ưu tiên so sánh của chúng bằng nhau. Hàm \`stable_sort\` bảo toàn thứ tự xuất hiện ban đầu của chúng trong vector gốc, tức là \`23\` xuất hiện trước \`32\`, trước \`41\`, trước \`14\`.
    *   Kết quả in ra màn hình chắc chắn là: \`23 32 41 14 15\`. Nếu ta dùng \`std::sort\` thông thường, thứ tự của 4 số này có thể bị xáo trộn.
...
> Nếu sử dụng toán tử chứa dấu bằng, khi mảng có các phần tử trùng nhau, thuật toán sắp xếp sẽ rơi vào trạng thái không xác định và gây lỗi tràn bộ nhớ hoặc treo chương trình.

> [!WARNING]
> **Bẫy 2 – Chọn sai loại thuật toán khi yêu cầu giữ thứ tự gốc:**
> Nhiều bạn nhầm tưởng chỉ cần thiết lập tiêu chí so sánh chặt chẽ là đủ, tuy nhiên đối với các bài toán yêu cầu giữ nguyên thứ tự ban đầu của các bản ghi có cùng độ ưu tiên, bạn buộc phải dùng \`std::stable_sort\` thay vì \`std::sort\` vì \`std::sort\` không có tính ổn định.
...
- [ ] Phân biệt rõ khi nào dùng \`std::sort\` và khi nào bắt buộc dùng \`std::stable_sort\`.`,
        homeworkProblems: [
          {
            id: "w5-l3-hw1",
            title: "Bài 1: Sắp xếp phân số",
            description: "Cho danh sách gồm N phân số. Hãy sắp xếp các phân số này theo giá trị thực tăng dần. Mỗi phân số được biểu diễn bởi tử số và mẫu số. Chú ý sử dụng kiểu cấu trúc `struct` để biểu diễn phân số và thực hiện so sánh chính xác (gợi ý: so sánh hai phân số $a/b$ và $c/d$ bằng cách so sánh tích nhân chéo $a \times d$ và $c \times b$ để tránh sai số khi chia số thực).",
            inputDesc: "Dòng 1: Số nguyên dương N (1 ≤ N ≤ 1000). N dòng tiếp theo, mỗi dòng chứa hai số nguyên dương lần lượt là tử số và mẫu số của một phân số (các giá trị tử số và mẫu số đều ≤ 1000).",
            outputDesc: "In ra N phân số sau khi đã sắp xếp tăng dần theo giá trị, mỗi phân số định dạng tử_số/mẫu_số trên một dòng.",
            sampleInput: "3\n1 2\n3 4\n1 3",
            sampleOutput: "1/3\n1/2\n3/4"
          },
          {
            id: "w5-l3-hw2",
            title: "Bài 2: Sắp xếp điểm tọa độ",
            description: "Cho N điểm trên mặt phẳng tọa độ Oxy. Hãy sắp xếp các điểm theo thứ tự hoành độ (x) tăng dần. Nếu hai điểm có cùng hoành độ, hãy sắp xếp theo tung độ (y) tăng dần. Sử dụng `struct` Point để biểu diễn tọa độ và hàm so sánh phù hợp.",
            inputDesc: "Dòng 1: Số nguyên dương N (1 ≤ N ≤ 10^{5}). N dòng tiếp theo, mỗi dòng chứa hai số nguyên x và y là tọa độ của một điểm (các giá trị x, y có giá trị tuyệt đối không vượt quá 10^{9}).",
            outputDesc: "In ra N dòng, mỗi dòng chứa hai số x và y biểu diễn điểm sau khi đã sắp xếp, cách nhau bởi một dấu cách.",
            sampleInput: "4\n2 3\n1 5\n2 1\n1 2",
            sampleOutput: "1 2\n1 5\n2 1\n2 3"
          },
          {
            id: "w5-l3-hw3",
            title: "Bài 3: Sắp xếp danh sách sản phẩm",
            description: "Một cửa hàng cần quản lý thông tin của N sản phẩm. Mỗi sản phẩm gồm có: Mã sản phẩm (chuỗi gồm tối đa 10 ký tự viết liền không chứa dấu cách), Đơn giá (số nguyên dương) và Số lượng tồn kho (số nguyên dương). Hãy sắp xếp các sản phẩm theo thứ tự giảm dần của Đơn giá. Nếu có nhiều sản phẩm có cùng đơn giá, hãy sắp xếp theo Số lượng tồn kho tăng dần. Nếu vẫn trùng nhau cả đơn giá và số lượng tồn kho, sắp xếp theo Mã sản phẩm tăng dần theo thứ tự từ điển.",
            inputDesc: "Dòng 1: Số nguyên dương N (1 ≤ N ≤ 1000). N dòng tiếp theo, mỗi dòng chứa ba thông tin của một sản phẩm: Mã sản phẩm, Đơn giá và Số lượng tồn kho.",
            outputDesc: "In ra danh sách các sản phẩm sau khi đã sắp xếp, mỗi sản phẩm gồm Mã sản phẩm, Đơn giá và Số lượng tồn kho trên một dòng, cách nhau bởi dấu cách.",
            sampleInput: "4\nSP01 150 10\nSP02 200 5\nSP03 150 20\nSP04 150 10",
            sampleOutput: "SP02 200 5\nSP01 150 10\nSP04 150 10\nSP03 150 20"
          },
          {
            id: "w5-l3-hw4",
            title: "Bài 4: Sắp xếp theo số lượng ước số",
            description: "Cho mảng gồm N số nguyên dương. Hãy sắp xếp các số trong mảng theo số lượng ước số tăng dần. Nếu hai số có cùng số lượng ước số, số nào có giá trị nhỏ hơn sẽ xếp trước. (Ví dụ: số 6 có các ước là {1, 2, 3, 6} (4 ước); số 9 có các ước là {1, 3, 9} (3 ước) nên số 9 sẽ xếp trước số 6).",
            inputDesc: "Dòng 1: Số nguyên N (1 ≤ N ≤ 1000). Dòng 2 chứa N số nguyên dương có giá trị không vượt quá 10^{6}.",
            outputDesc: "In ra dãy số đã được sắp xếp theo quy tắc trên, các số cách nhau bởi một khoảng trắng.",
            sampleInput: "5\n6 9 2 4 10",
            sampleOutput: "2 4 9 6 10"
          },
          {
            id: "w5-l3-hw5",
            title: "Bài 5: Sắp xếp phân đoạn",
            description: "Cho một dãy số gồm N số nguyên dương. Hãy thực hiện sắp xếp dãy số theo quy tắc sau:\n1. Số lẻ luôn đứng trước số chẵn.\n2. Các số lẻ được sắp xếp theo thứ tự giảm dần của giá trị.\n3. Các số chẵn được sắp xếp theo thứ tự tăng dần của tổng các chữ số của nó. Nếu hai số chẵn có cùng tổng các chữ số, số nào xuất hiện trước trong dãy ban đầu sẽ đứng trước (đây là tính chất sắp xếp ổn định).",
            inputDesc: "Dòng 1: Số nguyên N (1 ≤ N ≤ 1000). Dòng 2 chứa N số nguyên dương có giá trị không vượt quá 10^{9}.",
            outputDesc: "In ra dãy số sau khi đã sắp xếp theo quy tắc trên, các số cách nhau bởi một khoảng trắng.",
            sampleInput: "6\n12 7 9 24 1 20",
            sampleOutput: "9 7 1 20 12 24"
          }
        ]
      }    ]
  },
